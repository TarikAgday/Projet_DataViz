...
exports.getClubsNames = getClubsNames;
exports.getAgeCategories = getAgeCategories;
...
function getClubsNames(data) {
  // TODO: Return the neihborhood names
  var uniqueClubNames = _toConsumableArray(new Set(data.map(function (item) {
    return item.Club;
  })));
...
function getAgeCategories(number) {
  // TODO: Return the
  switch (number) {
    case "15" || "16":
      return 0;
...
function heatmapProcess(data) {
  ...
  console.log("Clubs", Clubs); // Loop through each club

  var _loop2 = function _loop2(i) {
    var data_per_club = data.filter(function (d) {
      return d.Club == Clubs[i];
    });
    console.log("DPC", data_per_club); // Designate the age groups

    var ageGroupSalaries = new Array(13).fill(0); // Each ageGroup contains two years; e.g 15-16, 17-18 [...] 39-40 years old

    for (var j = 0; j < data_per_club.length; j++) {
      var ageCategorie = getAgeCategories(data_per_club[j].Age);
      ageGroupSalaries[ageCategorie] += parseInt(Math.trunc(data_per_club[j].Salary.replace(',', '').substring(2)));
    }

    for (var k = 0; k < ageGroupSalaries.length; k++) {
      var ageGroup = k;
      clubResult.push({
        Club: Clubs[i],
        Age: ageGroup,
        ageGroupSalary: ageGroupSalaries[ageGroup]
      });
    }
 ...
 */
function updateGroupXScale(scale, data, width) {// TODO : Set the domain and range of the groups' x scale
}
/**
...
function updateYScale(scale, data, height) {// TODO : Set the domain and range of the graph's y scale
}
/**
...
exports.appendHeatMap = appendHeatMap;
...
function appendHeatMap(data) {
  ...
  var svg = d3.select("#viz_area_5").append("svg").attr("width", width + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); // Labels of row and columns

  var clubs = d3.map(data, function (d) {
    return d.Club;
  }).keys();
  var ageGroups = d3.map(data, function (d) {
    return d.Age;
  }).keys(); // Build X scales and axis:

  var xScale = d3.scaleBand().range([0, width]).domain(ageGroups).padding(0.01);
  svg.append("g").attr("transform", "translate(0," + height + ")").call(d3.axisBottom().scale(xScale)).selectAll("text").attr("transform", "translate(-10 20) rotate(-90)"); // Build Y scales and axis:

  var yScale = d3.scaleBand().range([height, 0]).domain(clubs).padding(0.01);
  svg.append("g").call(d3.axisLeft().scale(yScale)); // Build color scale

  var myColor = d3.scaleLinear().domain([0, 1000000000]).range(["white", "#69b3a2"]); // create a tooltip

  var tooltip = d3.select("#viz_area_5").append("div").style("opacity", 0).attr("class", "tooltip").style("background-color", "blue").style("padding", "1px"); // Three function that change the tooltip when user hover / move / leave a cell
  ...
  var mousemove = function mousemove(d) {
    tooltip.html("The exact value of<br>this cell is: " + d.ageGroupSalary).style("left", d3.mouse(this)[0] + 70 + "px").style("top", d3.mouse(this)[1] + "px");
  };
  ...
  d3.select("#viz_area_5").selectAll(".salary-g").data(data).join("g").attr("class", "salary-g").append("rect").attr("class", "salary-rect").attr("x", function (d) {
    return xScale(d.Age);
  }).attr("y", function (d) {
...
},{"d3":"../node_modules/d3/index.js","./preprocess":"scripts/preprocess.js"}],"scripts/legend.js":[function(require,module,exports) {
"use strict";
 ...
 */
function draw(data, color) {// TODO : Generate the legend in the div with class "legend". Each SVG rectangle
...
$(function () {
  Promise.all([d3.csv("StatsJoueursConv.csv"), d3.csv("ClassementParEquipeConv.csv")]).then(function (files) {
    ...
    heatmap.appendHeatMap(dataHeatMap); // heatmap.appendHeatMap(dataHeatMap)
...
},{"./scripts/preprocess.js":"scripts/preprocess.js","./scripts/viz.js":"scripts/viz.js","./scripts/heatmap.js":"scripts/heatmap.js","./scripts/legend.js":"scripts/legend.js","./scripts/tooltip.js":"scripts/tooltip.js","d3-tip":"../node_modules/d3-tip/index.js"}],"../../../../../../../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "57214" + '/');
